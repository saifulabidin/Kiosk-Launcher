    ...
    companion object {
        fun startKioskMode(context: Activity) {
            ...
            if (devicePolicyManager.isDeviceOwnerApp(context.packageName)) {
                ...
                // Obter apps selecionados
                val prefs = context.getSharedPreferences("kiosk_prefs", Context.MODE_PRIVATE)
                val selectedApps = prefs.getStringSet("selected_apps", setOf())?.toMutableSet()
                    ?: mutableSetOf()

                // Adicionar o próprio app à whitelist
                selectedApps.add(context.packageName)
                ...
                Toast.makeText(
                    context,
                    "This app is not an owner device",
                    Toast.LENGTH_SHORT
                ).show()
            }
        }
